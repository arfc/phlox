...
moder[] = Extrude {0, 0, 20} { Surface{6}; };
Physical Surface('moderator_top') = {moder[0]};
Physical Volume('moderator') = {moder[1]};
Physical Surface('fuel_bottom') = {1, 2, 3, 4};
fuel_vol[] = {};
fuel_surf[] = {};
fuel[] = Extrude {0, 0, 20} { Surface{1}; };
fuel_vol += fuel[1];
fuel_surf += fuel[0];
fuel[] = Extrude {0, 0, 20} { Surface{2}; };
fuel_vol += fuel[1];
fuel_surf += fuel[0];
fuel[] = Extrude {0, 0, 20} { Surface{3}; };
fuel_vol += fuel[1];
fuel_surf += fuel[0];
fuel[] = Extrude {0, 0, 20} { Surface{4}; };
fuel_vol += fuel[1];
fuel_surf += fuel[0];
